...
namespace BSEBResultBlockchainAPI.Services
{
    public class ResultPublishService : IResultPublishService
    {
        ...
        public async Task PublishAllResultsAsync(CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                foreach (var batch in batches)
                {
                    ...
                    _logger.LogInformation("Batch completed → BatchSize={BatchSize}, TotalProcessed={Processed}, Skipped={Skipped}, Failed={Failed}",batch.Count,processed,skipped,failed);
        ...
        private async Task<ProcessResult> ProcessSingleRollAsync(string rollCode, string rollNo)
        {
            try
            {
                ...
                //{
                    ...
                    await _flureeService.SaveEncV1RecordAsync(rollCode, rollNo, encrypted);
                ...
                _logger.LogInformation("rollcode={RollCode} rollnumber={RollNo}", rollCode, rollNo);
        ...
        public async Task Encv2PublishAllResultsAsync(CancellationToken cancellationToken = default)
        {
            try
            {
                _logger.LogInformation("=== Starting BSEB Result Blockchain Publish ===");
                ...
                foreach (var batch in batches)
                {
                    ...
                    var tasks = batch.Select(async roll =>
                    {
                        ...
                        try
                        {
                            var result = await EncV2ProcessSingleRollAsync(roll.RollCode, roll.RollNo);
        ...
        private async Task<ProcessResult> EncV2ProcessSingleRollAsync(string rollCode, string rollNo)
        {
            try
            {
                // ── Step 1: Get student result data from SQL DB ──────────────────────
                ...
                // ── Step 2: Encrypt student data ─────────────────────────────────────
                string Enc_V2 = QrUtility.GenerateEncrypteForstudentdata(student);

                // ── Step 3: Check Fluree for existing record ──────────────────────────
                // ✅ Fetch approvals once
                //var approvals = await _dbHelper.GetAllAroval();

                //// ✅ Convert to lookup for fast matching
                //var approvalLookup = approvals.ToDictionary(x => (x.RollCode, x.RollNo), x => (x.Approval1, x.Approval2));
                var existing = await _flureeService.GetByRollAsync(rollCode, rollNo);

                if (existing != null)
                {
                    //var Approval = await _dbHelper.GetApproval();
                    // ✅ CASE A: No record in Fluree → First time INSERT
                    // Fluree will store: encrypteddata = '["ENC_v1"]'
                    //await _flureeService.SaveNewRecordAsync(rollCode, rollNo, encrypted);
                    //  await _flureeService.SaveEncV1RecordAsync(rollCode, rollNo, encrypted);
                    //if (existing.approval1 == "Approved" && existing.approval2 == "Approved")
                    //{
                    //    await _flureeService.SaveEncV2RecordAsync(rollCode, rollNo, encrypted);
                    //}
                    var approvals = await _dbHelper.GetAllAroval(rollCode, rollNo);

                    if (approvals == null || approvals.Count == 0)
                    {
                        _logger.LogWarning("[Skip] No approvals found → {RollCode}/{RollNo}", rollCode, rollNo);
                        return ProcessResult.Skipped;
                    }
